# Install slineplus — full statusline with state files for Windows (PowerShell)
# Run from the repo root: powershell -File install_plus.ps1
# Copies slineplus.ps1 + statusline.ini to ~/.claude/statusline/
...
$files = @(
    @{Name="slineplus.ps1"; From=Join-Path $scriptRoot 'slineplus.ps1'},
...
$commandPath = ($TargetDir.Replace('\', '/') + '/slineplus.ps1')
$newStatusLine = @{
    type    = 'command'
