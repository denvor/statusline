...
[![version](https://img.shields.io/badge/version-1.0.0-green.svg)](CHANGELOG_zh.md) [![en](https://img.shields.io/badge/lang-English-red.svg)](README.md)
...
> **第三方 API 注意：** 部分提供商会在模型名后追加 `[1m]` 或 `[1M]` 后缀表示 1M 上下文窗口。脚本在匹配前会自动去除此后缀，因此 INI 段应使用基础模型名（如 `[deepseek-v4-pro]`，而非 `[deepseek-v4-pro [1m]]`）。
...
```ini
[display]
# 可选字段: project, model, thinking, effort, bar, ctx, call, git, time, cost
order = project, model, thinking, effort, bar, ctx, call, git, time, cost
```

| 键 | 说明 | 默认值 |
|----|------|--------|
| `order` | 逗号分隔的字段名列表，按此顺序显示 | 全部 10 个字段 |

未在 `order` 中列出的字段会被隐藏。可见字段之间自动插入 `|` 分隔符。
